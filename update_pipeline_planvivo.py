...
BASE_DIR = "app/data/PLAN_VIVO"
CHROMA_DIR = "app/embeddings_plan_vivo"
TOKEN_LIMIT = 280_000
...
def load_docs_from_subfolder(folder_name, doc_type):
    ...
    for root, _, files in os.walk(folder_path):
        for fname in files:
            ...
            try:
                loaded = loader.load()
                relative_project_path = os.path.relpath(root, BASE_DIR)
                project_id = os.path.basename(relative_project_path)
                for i, doc in enumerate(loaded):
                    doc.metadata.update({
                        "source": fname,
                        "page": i + 1,
                        "clause": extract_clause_number(doc.page_content),
                        "project_type": "PLAN_VIVO",
